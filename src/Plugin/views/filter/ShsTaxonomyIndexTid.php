...
/**
 * @file
 * Contains \Drupal\shs\Plugin\views\filter\ShsTaxonomyIndexTid.
 */
...
class ShsTaxonomyIndexTid extends TaxonomyIndexTid {
  ...
  protected function valueForm(&$form, FormStateInterface $form_state) {
    ...
    $hooks = [
      'shs_js_settings',
      "shs_{$this->definition['field_name']}_js_settings",
      "shs_{$this->view->id()}__{$identifier}_js_settings",
      "shs_{$this->view->id()}__{$this->definition['field_name']}_js_settings",
      "shs_{$this->view->id()}__{$this->view->current_display}__{$identifier}_js_settings",
      "shs_{$this->view->id()}__{$this->view->current_display}__{$this->definition['field_name']}_js_settings",
    ];
    // Allow other modules to override the settings.
    \Drupal::moduleHandler()->alter($hooks, $settings_shs, $bundle, $this->definition['field_name']);
    ...
    $element_key = Html::getUniqueId(sprintf('shs-%s', $this->definition['field_name']));
    $form['value']['#attributes'] = array_merge($form['value']['#attributes'], [
      'data-shs-selector' => $element_key,
    ]);

    $form['value']['#shs'] += [
      'classes' => shs_get_class_definitions($form['value']['#field_name']),
    ];
    $form['value']['#attached'] = array_merge($form['value']['#attached'], [
      'drupalSettings' => [
        'shs' => [
